...
{
  servoRight = (m_Hello.forward() + 1509) * 2 + SERVO_MIN;
  servoLeft = (m_Hello.forward() + 1509) * 2 + SERVO_MIN; 

  if(m_Hello.direction() > 1498)
  {
	servoRight += ((m_Hello.direction() - 1498) / 2);
  }
  else 
  {
	servoLeft += ((m_Hello.direction() - 1498) / 2);
  }
}
